...
void padding(const uint8_t* message, size_t message_len, uint8_t* padded_message, size_t& padded_message_len,size_t m) {
	size_t remainder = message_len % BLOCK_SIZE;
	size_t remainder_m = m % BLOCK_SIZE;
	size_t padding_len = (remainder < 56) ? (56 - remainder) : (120 - remainder);
	size_t padding_len_m = (remainder_m < 56) ? (56 - remainder_m) : (120 - remainder_m);
	padded_message_len = message_len + padding_len + 8;
	size_t padded_message_m_len = m + padding_len_m + 8;

	memcpy(padded_message, message, message_len);
	padded_message[message_len] = 0x80;
	memset(padded_message + message_len + 1, 0, padding_len - 1);

	uint64_t message_bits = (uint64_t)(message_len + padded_message_m_len) * 8;
...
void len_ex_attack_SM3(const uint8_t* message, uint32_t* digest,uint32_t* hash,size_t m) {
	...
	padding(message, message_len, padding_message, padding_message_len,m);
